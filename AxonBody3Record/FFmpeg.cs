...
namespace AxonBody3Record
{
    class FFmpeg
    {
        ...
        static string ffmpegPath = "E:\\DownLoad\\qb-AxonBody3Camera\\ffmpeg-master-latest-win64-gpl\\bin\\ffmpeg.exe";
        //static string ffmpegPath = @"ffmpeg-master-latest-win64-gpl\bin\ffmpeg.exe";
        ...
        public static void Start(string outFilePath)
        {
            ...
            string arguments = "-f gdigrab -framerate 24 -r 15 -i desktop -pix_fmt yuv420p -f dshow -i audio=" + "\"virtual-audio-capturer\"" + " -vcodec" + " h264_qsv" + " \"" + outFilePath + "\"";
            /*转码，
            * 视频录制设备：gdigrab；
            * 录制对象：整个桌面desktop；
            * 音频录制方式：dshow；
